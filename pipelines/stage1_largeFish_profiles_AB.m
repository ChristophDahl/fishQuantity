%% analysisFishQuantity8b.m
% Large-fish compartment profiles (Stage-1 descriptive figure).
%
% Panel A: equal-number trials (diagonal contrasts) with per-trial "bias correction"
%          (flip profile if occupancy is higher on the left).
% Panel B: all contrasts (raw, uncorrected), using the canonical contrast ordering.
%
% Outputs:
%   - Figure: LargeFish_compartmentProfiles_AB.png (saved to figurefolder)
%   - CSV:    fit_parameters_large.csv             (QC / SI table)
%   - MAT:    profiles_large.mat                   (means/SEMs/Ns/labels for downstream use)
%   - MAT:    behavior_trials_large.mat            (all_subjects snapshot for reproducibility)
%
...
% Canonical contrast order (left_right), then tag with '_l' for LARGE fish
desired_order = { ...
...
results_raw  = extract_profiles(resultsfolder, all_subjects, subject_ids, roi_rect, n_bins, labels_large, false);
...
xlabels_raw = labels_large;            % preserved order
...
row_names = { ...
    {'1_1_l','1_2_l','1_4_l','1_8_l','1_16_l'}, ...
    {'2_2_l','2_4_l','2_8_l','2_16_l'}, ...
    {'4_4_l','4_8_l','4_16_l'}, ...
    {'8_8_l','8_16_l'}, ...
    {'16_16_l'} ...
    };
...
saveas(hFig, fullfile(figurefolder, 'LargeFish_compartmentProfiles_AB.png'));
...
writetable(fit_table, fullfile(resultsfolder, 'fit_parameters_large.csv'));
disp(fit_table);

% Keep a tidy MAT with means/SEMs/Ns/labels for downstream use
profiles_large = struct( ...
    'labels',        labels_large, ...
    'means_raw',     means_raw, ...
    'sems_raw',      sems_raw, ...
    ...
    'n_bins',        n_bins, ...
    'roi_width',     roi_width);
save(fullfile(resultsfolder,'profiles_large.mat'), 'profiles_large');

% Also keep a reference to the original per-subject trial tables if needed later
trials_large = all_subjects;   % rename for clarity
save(fullfile(resultsfolder,'behavior_trials_large.mat'),'trials_large');

%% ----------------------- helpers -----------------------
function out = extract_profiles(resultsfolder, all_subjects, subject_ids, roi_rect, n_bins, labels_any, do_bias_correction)
