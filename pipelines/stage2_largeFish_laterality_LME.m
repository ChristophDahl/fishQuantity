%% lateralityIndexLargeFishQuantity2.m
% Compute per-trial laterality index (LI) for LARGE fish trials and fit a per-trial
...
%   - laterality_trials_large.mat        (struct array all_trials)
%   - fixed_effects_large.csv            (fixed-effect table for manuscript)
%   - lme_large.mat                      (fitted model + R2)
%   - pred_line_large.csv                (population prediction line)
%
% Laterality analysis (LARGE fish): per-trial LI + mixed-effects model
% Keeps your existing Panel C at subplot(5,6,18) unchanged.
...
%% --------- Build per-trial LI (LARGE fish), robust trial↔detection pairing ----------
for s = 1:length(all_subjects)
    ...
    for tIdx = 1:nUse
        ...
        % LARGE-fish only in this script
        if ~strcmp(size_letter,'l'), continue; end
...
% Save per-trial data for reproducibility
if ~exist(resultsfolder, 'dir'); mkdir(resultsfolder); end
save(fullfile(resultsfolder, 'laterality_trials_large.mat'), 'all_trials');

%% ---------- Panel C (KEEP AS IS): aggregate LI by absolute N on right ----------
abs_numbers   = [1 2 4 8 16];
lat_by_number = cell(numel(abs_numbers),1);

for i = 1:numel(all_trials)
    t = all_trials(i);
    % Use int2 as "right side" group size
...
% ----------- Plot by absolute number of fish (unchanged panel C) --------------
subplot(5,6,23)
xa = abs_numbers;
meanx = mean_lat;
stderrorx = sem_lat;
plot(xa, meanx, '-', 'LineWidth', 0.75, 'Color', [0 0 0]); hold on
...
set(gca, 'Box','off','TickDir','out','TickLength',[.01 .01], ...
  ...
  'XTick',[1,2,4,8,16], 'YTick', -.2:.1:.2, 'LineWidth', .75);
axis square
axis([0.5 16.5 -.2 .2])
text(-4.5, .20, 'E', 'Fontsize',14, 'FontWeight', 'bold');

%% ---------- Primary analysis: per-trial mixed-effects model (LARGE fish) ----------
T = struct2table(all_trials);
T = T(strcmp(T.size_letter,'l') & T.int1>0 & T.int2>0, :);

% Predictor: log2(N_right/N_left) (0..4). For plotting we'll use both 0..4 and -4..0 styles.
T.log2_ratio = log2(T.int2 ./ T.int1);
T.subject    = categorical(T.subject);

% Mixed-effects model: LI ~ log2_ratio + (1|subject)
lme_large = fitlme(T, 'lat_idx ~ 1 + log2_ratio + (1|subject)');

% Fixed effects summary
fe  = lme_large.Coefficients;
ci  = coefCI(lme_large);
...
try
    R2m = lme_large.Rsquared.Marginal;
    R2c = lme_large.Rsquared.Conditional;
catch
    [R2m, R2c] = pseudoR2_Nakagawa(lme_large, T);
end

% Console report
disp(lme_large);
fprintf('Large fish: \x03B2 per doubling (log2_ratio) = %.4f  (95%% CI [%.4f, %.4f]), p=%.4g\n', ...
    beta, beta_ciL, beta_ciU, beta_p);
fprintf('R2_marginal=%.3f, R2_conditional=%.3f\n', R2m, R2c);

% Save stats for manuscript
fixed_effects_large = table(fe.Name, fe.Estimate, fe.SE, fe.DF, fe.tStat, fe.pValue, ...
    'VariableNames', {'Term','Estimate','SE','DF','t','p'});
fixed_effects_large.CI_low  = ci(:,1);
fixed_effects_large.CI_high = ci(:,2);
writetable(fixed_effects_large, fullfile(resultsfolder,'fixed_effects_large.csv'));
save(fullfile(resultsfolder,'lme_large.mat'), 'lme_large','fixed_effects_large','R2m','R2c');
...
yhat    = predict(lme_large, newT, 'Conditional', false);

% Flip x to -4..0 variant if you need it later:
xx_plot = -xx_pos; % -4..0 = log2(min/max)
pred_large = table(xx_plot, yhat, 'VariableNames', {'log2ratio_plot','LI_hat'});
writetable(pred_large, fullfile(resultsfolder,'pred_line_large.csv'));

%% ---------- Panel H: LI vs log2(Right/Left) with LME population fit ----------
% Compact scatter: condition means (inference comes from per-trial LME)
...
for i = 1:numel(equalNs)
    N = equalNs(i);
    % equal numbers, LARGE fish only
    mask = [all_trials.int1] == N & [all_trials.int2] == N ...
         & strcmp({all_trials.size_letter}, 'l');
    if ~any(mask), continue; end
    ati = all_trials(mask);
...
% select unequal-number trials, LARGE fish only
maskUnequal = strcmp({all_trials.size_letter}, 'l') & [all_trials.int1] > 0 & [all_trials.int2] > 0 ...
...
% text(-70, -10, 'C', 'Fontsize',14, 'FontWeight','bold');
% text(5, 280, 'Stimulus dimensions:', 'FontSize', 9);
% text(5, 330, 'Length: 47mm', 'FontSize', 9);
% text(5, 380, 'Width: 18mm', 'FontSize', 9);
