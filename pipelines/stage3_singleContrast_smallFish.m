...
% Single-contrast analysis: 0 vs N compartment occupancy profiles (small stimuli)
%
...
[results_raw, all_laterality] = extract_profiles(resultsfolder, all_subjects, subject_ids, roi_rect, n_bins, labels_small, false);
results_corr = extract_profiles(resultsfolder, all_subjects, subject_ids, roi_rect, n_bins, labels_small, true);

means_raw = results_raw.means;
sems_raw = results_raw.sems;
xlabels_raw = labels_small; % order is preserved
...
fprintf('\nCurve fits for 0 vs N (small, raw means):\n');
fprintf('N\tSlope\tCurvature\tR2\n');

for i = 1:numel(labels_small)
    ...
    % Save linear fit
    curvefit_results(i).label = labels_small{i};
...
for i = 1:numel(labels_small)
    ...
    R2_1 = 1 - sum((y - y1).^2)/sum((y-mean(y)).^2);
    R2_2 = 1 - sum((y - y2).^2)/sum((y-mean(y)).^2);
    curvefit_corr_results(i).label = labels_small{i};
...
writetable(T_curvefits_corr, fullfile(resultsfolder, 'curvefits_zero_vs_N_small_corr.csv'));

% save(fullfile(resultsfolder, 'laterality_zero_vs_small.mat'), 'all_laterality', 'labels_small');
save(fullfile(resultsfolder, 'laterality_zero_vs_small.mat'), ...
     'all_laterality', ...
     'labels_small', ...
     'curvefit_results', ...
...
%     end
    ...
    for c = 1:n_this_row
        ...
        subplot(n_rows*2, n_cols + 2, (r-1)*(n_cols) + c + 2);
        if ~isempty(idx_raw)
        ...
        if c == 1 & r == 1
            text(-2, .5, 'C', 'Fontsize',14, 'FontWeight', 'bold')
        end
    end
end
...
text(-60, -80, 'A', 'Fontsize',14, 'FontWeight','bold');
text(5, 120, 'Stimulus dimensions:', 'FontSize', 9);
text(5, 160, 'Length: 25mm', 'FontSize', 9);
text(5, 200, 'Width: 6mm', 'FontSize', 9);
...
text(-220, -210, 'B', 'Fontsize',14, 'FontWeight','bold');
...
function [out, all_laterality] = extract_profiles(resultsfolder, all_subjects, subject_ids, roi_rect, n_bins, labels_small, do_bias_correction)

    n = length(labels_small);
